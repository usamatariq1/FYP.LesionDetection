    ...
    test_app.findOne({email:req.body.email,test_name:req.body.test_name},function(err,docs){
        if(err)
        {
            ...
            if(docs){
                ...
                test_app.find({date:date,test_name:req.body.test_name},function(err,docs){
                    if(docs){
                        tests.find({
                            test_name: req.body.test_name
                        }).populate('Test').exec(function (error, results) {
                            if (error) {
                                console.log("Error in line 63")
                                return next(error);
                            }
                    ...
                    // Respond with valid data
                                test_frequency=results[0].test_frequency;
                                if(docs.length<test_frequency){
